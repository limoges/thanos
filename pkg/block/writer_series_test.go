...
func TestSeriesWriter_WriteSeries_e2e(t *testing.T) {
	...
	for _, tcase := range []struct {
		...
		t.Run(tcase.name, func(t *testing.T) {
			...
			s := NewSeriesWriter(tmpDir, logger, chunkPool)
			...
			id := ulid.MustNew(uint64(len(blocks)+1), nil)
			d, err := NewDiskWriter(ctx, logger, filepath.Join(tmpDir, id.String()))
			testutil.Ok(t, err)
			if tcase.expectedErr != nil {
				err := s.WriteSeries(ctx, nil, d)
				testutil.NotOk(t, err)
				testutil.Equals(t, tcase.expectedErr.Error(), err.Error())
				return
			}
			testutil.Ok(t, s.WriteSeries(ctx, blocks, d))
...
func readBlockSeries(t *testing.T, bDir string) []seriesSamples {
	indexr, err := index.NewFileReader(filepath.Join(bDir, IndexFilename))
	testutil.Ok(t, err)
	defer indexr.Close()

	chunkr, err := chunks.NewDirReader(filepath.Join(bDir, ChunksDirname), nil)
	testutil.Ok(t, err)
	defer chunkr.Close()
...
func createBlockSeries(bDir string, inputSeries []seriesSamples) (err error) {
	d, err := NewDiskWriter(context.TODO(), log.NewNopLogger(), bDir)
	if err != nil {
		return err
	}
	defer func() {
